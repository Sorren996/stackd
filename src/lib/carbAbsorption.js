...
$$\text{Correction Insulin} = \frac{\max(0, \text{Current Glucose} - 110)}{50}$$

$$\text{Food Insulin} = \max(0, \text{Active Insulin} - \text{Correction Insulin})$$

$$\text{Net Active Carbs} = \text{Active Carbs} - (\text{Food Insulin} \times 10)$$


const netActiveCarbs = useMemo(() => {
  const currentGlucose = latestGlucose ? latestGlucose.value : 100;
  const targetGlucose = 110; // Target blood glucose
  const isf = 50;           // Insulin Sensitivity Factor (1u lowers glucose by 50 mg/dL)
  
  // Insulin dedicated to correcting high blood sugar, not covering carbs
  const activeCorrectionUnits = Math.max(0, currentGlucose - targetGlucose) / isf;
  
  // Active insulin left to cover carbohydrates
  const activeFoodUnits = Math.max(0, activeUnits - activeCorrectionUnits);
  
  // Balanced calculation (assuming 1u covers 10g of carbs)
...
{ name: "Chicken Tacos", carbs: 22, gi: 52, category: "Medium Absorbing", profile: "medium" }
...
{ name: "Chocolate Chip Cookie", carbs: 20, gi: 65, category: "Fast Absorbing", profile: "fast" }
{ name: "Pancakes with Syrup", carbs: 45, gi: 72, category: "Fast Absorbing", profile: "fast" }
...
{ name: "Greek Yogurt with Berries & Walnuts", carbs: 14, gi: 25, category: "Slow Absorbing", profile: "slow" },
...
{ name: "Apple Slides with Peanut Butter", carbs: 18, gi: 38, category: "Medium Absorbing", profile: "medium" },
...
{ name: "Sweetened Cereal (e.g. Frosted Flakes)", carbs: 26, gi: 80, category: "Fast Absorbing", profile: "fast" },
{ name: "White Toast with Strawberry Jam", carbs: 28, gi: 75, category: "Fast Absorbing", profile: "fast" },

// Sweet Treats & Quick Sugars
