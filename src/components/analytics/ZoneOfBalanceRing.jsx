...
function BreakdownItem({ value, label, color }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span className="text-[10px] uppercase tracking-[0.14em] text-white/30">{label}</span>
    </div>
  );
}
    ...
    <div className="relative">
      ...
      >
        <div
          ...
          style={{
            background: "radial-gradient(circle, rgba(91,168,138,0.07) 0%, rgba(91,168,138,0.015) 45%, transparent 70%)",
            filter: "blur(8px)",
          }}
        />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center">
        {/* Hero donut + percentage */}
        <div className="relative">
          <svg width="180" height="180" viewBox="0 0 180 180" style={{ transform: "rotate(-90deg)" }}>
            <circle cx="90" cy="90" r={radius} fill="none" stroke="rgba(255,255,255,0.035)" strokeWidth="16" />
            {belowPercent > 0 && (
              <motion.circle
            ...
            {inRangePercent > 0 && (
              <motion.circle
                ...
                style={{ filter: "drop-shadow(0 0 5px rgba(91,168,138,0.18))" }}
              />
            )}
            {abovePercent > 0 && (
              <motion.circle
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <motion.span
              ...
              className="text-[3.25rem] font-black leading-none tracking-tight text-white"
            >
              {Math.round(inRangePercent)}%
            </motion.span>
            <span className="mt-2 text-[11px] text-white/35">In comfort zone</span>
